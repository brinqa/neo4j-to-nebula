...
/**
 * Configuration object containing settings for transactions. Instances are immutable and can be
 * reused for multiple transactions.
 *
 * <p>Configuration is supported for:
 *
 * <ul>
 *   <li>queries executed in auto-commit transactions - using various overloads of {@link
 *       Session#run(String, TransactionConfig)} and {@link AsyncSession#runAsync(String,
 *       TransactionConfig)}
 *   <li>transactions started by transaction functions - using {@link
 *       Session#readTransaction(TransactionWork, TransactionConfig)}, {@link
 *       Session#writeTransaction(TransactionWork, TransactionConfig)}, {@link
 *       AsyncSession#readTransactionAsync(AsyncTransactionWork, TransactionConfig)} and {@link
 *       AsyncSession#writeTransactionAsync(AsyncTransactionWork, TransactionConfig)}
 *   <li>unmanaged transactions - using {@link Session#beginTransaction(TransactionConfig)} and
 *       {@link AsyncSession#beginTransactionAsync(TransactionConfig)}
 * </ul>
 *
 * <p>Creation of configuration objects can be done using the builder API:
 *
 * <pre>{@code
...
public class TransactionConfig implements Serializable {
  ...
  public static class Builder {
    ...
    /**
     * Set the transaction timeout. Transactions that execute longer than the configured timeout
     * will be terminated by the database.
     *
     * <p>This functionality allows to limit query/transaction execution time. Specified timeout
     * overrides the default timeout configured in the database using {@code
     * dbms.transaction.timeout} setting.
     *
     * <p>Provided value should not be {@code null} and should not represent a duration of zero or
     * negative duration.
     *
    ...
    /**
     * Set the transaction metadata. Specified metadata will be attached to the executing
     * transaction and visible in the output of {@code dbms.listQueries} and {@code
     * dbms.listTransactions} procedures. It will also get logged to the {@code query.log}.
     *
     * <p>This functionality makes it easier to tag transactions and is equivalent to {@code
     * dbms.setTXMetaData} procedure.
     *
     * <p>Provided value should not be {@code null}.
     *
    ...
    public Builder withMetadata(Map<String, Object> metadata) {
      ...
      metadata
          ...
          .forEach(
              Extract
                  ::assertParameter); // Just assert valid parameters but don't create a value map
      // yet
