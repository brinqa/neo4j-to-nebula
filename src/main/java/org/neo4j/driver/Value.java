 ...
 *
 * <p>This interface describes a number of <code>isType</code> methods along with <code>typeValue
 * </code> methods. The first set of these correlate with types from the Neo4j Type System and are
 * used to determine which Neo4j type is represented. The second set of methods perform coercions to
 * Java types (wherever possible). For example, a common String value should be tested for using
 * <code>isString</code> and extracted using <code>stringValue</code>.
 *
 * <h2>Navigating a tree structure</h2>
 *
 * Because Neo4j often handles dynamic structures, this interface is designed to help you handle
 * such structures in Java. Specifically, {@link Value} lets you navigate arbitrary tree structures
 * without having to resort to type casting.
 *
 * <p>Given a tree structure like:
 *
 * <pre>{@code
 * {
 *   users : [
 ...
 *
 * <pre class="docTest:ValueDocIT#classDocTreeExample">{@code
 * String username = value.get("users").get(1).get("name").asString();
 * }</pre>
 *
 * You can also easily iterate over the users:
 *
 * <pre class="docTest:ValueDocIT#classDocIterationExample">{@code
...
public interface Value extends MapAccessor, MapAccessorWithDefaultValue {
   ...
   * @return the value or a {@link org.neo4j.driver.internal.value.NullValue} if the index is out of
   *     bounds
   ...
   * </ul>
   *
   * Note that the types in {@link TypeSystem} refers to the Neo4j type system where {@link
